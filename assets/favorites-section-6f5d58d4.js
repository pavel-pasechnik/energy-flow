import"./api-energy-flow-3175d380.js";import"./vendor-cae18210.js";const f=document.querySelector(".favorites-list-item"),b=document.querySelector(".favorites-container-error-notification"),u=document.querySelector(".favorites-main"),g=document.querySelector(".favorites-pagination-container-btn"),w=window.location.href,L="http://localhost:5173/favorites.html";function y(){return w===L}const x=y();async function k(){if(x){let n=function(){let i=JSON.parse(localStorage.getItem("favorites"));if(i.length===0||i===null)return u.remove(),h();console.log(i);const l=8;let s=1;if(window.matchMedia("(max-width: 376px)").matches){let o=function(e,t,a){f.innerHTML="",a--;const v=t*a,d=v+t,p=e.slice(v,d);f.innerHTML=m(p)},r=function(e,t){g.innerHTML="";const a=document.querySelector(".pagination"),v=Math.ceil(e.length/t);for(let d=0;d<v;d++){const p=c(d+1);g.appendChild(p)}a.appendChild(g)},c=function(e){const t=document.createElement("li");return t.innerText=e,s==e&&t.classList.add("favorites-pagination-btn"),s!==e&&t.classList.add("favorites-pagination-btn-not-activ"),t.addEventListener("click",()=>{s=e,o(i,l,s);let a=document.querySelector("li.favorites-pagination-btn");a.classList.remove("favorites-pagination-btn"),a.classList.add("favorites-pagination-btn-not-activ"),t.classList.remove("favorites-pagination-btn-not-activ"),t.classList.add("favorites-pagination-btn")}),t};o(i,l,s),r(i,l)}else f.innerHTML=m(i);f.addEventListener("click",o=>{const r=o.target;if(i.length===0||i===null)return u.classList.add("is-hidden"),h();if(r.classList.contains("favorites-btn-trash")){let c=r.dataset.id;const e=i.filter(t=>t._id!==c);localStorage.setItem("favorites",JSON.stringify(e)),console.log(c),n()}})};u.classList.remove("is-hidden"),n(),window.addEventListener("resize",n)}else return}function m(n){return n.reduce((i,{bodyPart:l,burnedCalories:s,target:o,time:r,equipment:c,_id:e})=>i+`<li class="favorits-item">
          <div class="favorites-container-nav">
            ...
            <div class="favorites-container-start">
              ...
              <button type="button" class="favorites-btn-arrow" data-action="${e}">
...
//# sourceMappingURL=favorites-section-6f5d58d4.js.map
