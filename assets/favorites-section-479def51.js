const u=document.querySelector(".favorites-list-item"),k=document.querySelector(".favorites-container-error-notification"),h=document.querySelector(".favorites-main"),b=document.querySelector(".favorites-pagination-container-btn");function x(){const f=window.location.href,o="favorites";return f.includes(o)}const C=x();async function B(){if(C){let a=function(e){if(console.log(e),e.length===0||e===null){h.remove(),M();return}const r=8;let n=1;if(window.matchMedia("(max-width: 376px)").matches){let c=function(i,t,s){u.innerHTML="",s--;const p=t*s,v=p+t,m=i.slice(p,v);u.innerHTML=L(m)},l=function(i,t){b.innerHTML="";const s=document.querySelector(".pagination"),p=Math.ceil(i.length/t);for(let v=0;v<p;v++){const m=d(v+1);b.appendChild(m)}s.appendChild(b)},d=function(i){const t=document.createElement("li");return t.innerText=i,n==i&&t.classList.add("favorites-pagination-btn"),n!==i&&t.classList.add("favorites-pagination-btn-not-activ"),t.addEventListener("click",()=>{n=i,c(e,r,n);let s=document.querySelector("li.favorites-pagination-btn");s.classList.remove("favorites-pagination-btn"),s.classList.add("favorites-pagination-btn-not-activ"),t.classList.remove("favorites-pagination-btn-not-activ"),t.classList.add("favorites-pagination-btn")}),t};var w=c,g=l,y=d;c(e,r,n),l(e,r)}else u.innerHTML=L(e);u.addEventListener("click",c=>{const l=c.target;if(e.length===0||e===null){h.classList.add("is-hidden"),M();return}if(l.classList.contains("favorites-btn-trash")){let d=l.dataset.id;const i=e.filter(t=>t._id!==d);localStorage.setItem("favorites",JSON.stringify(i)),console.log(d),a(i)}})};var f=a;h.classList.remove("is-hidden");let o=JSON.parse(localStorage.getItem("favorites"))||[];a(o)}else return}function L(f){return f.reduce((o,{bodyPart:a,burnedCalories:e,target:r,time:n,equipment:w,_id:g})=>o+`<li class="favorits-item">
...
//# sourceMappingURL=favorites-section-479def51.js.map
